    ...
    fun initUserInfo(){
        ...
        UserService().getUsers(users.id, object : RetrofitCallback<HashMap<String, Any>> {
            ...
            override fun onSuccess(code: Int, responseData: HashMap<String, Any>) {
                ...
                //json : responseData["order"]

                val orderList:List<Order> = Gson().fromJson(responseData["order"].toString(), Array<Order>::class.java).toList()
                for(i in orderList){
                    Log.d(TAG, "onSuccess: ${i.orderTable}")
                }
                var pay = user.money
